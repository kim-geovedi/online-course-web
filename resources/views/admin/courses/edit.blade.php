            ...
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.courses.update', $course) }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ $course->name }}" autofocus autocomplete="name" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="path_trailer" :value="__('path_trailer')" />
                        <x-text-input id="path_trailer" class="block mt-1 w-full" type="text" name="path_trailer" value="{{ $course->path_trailer }}" autofocus autocomplete="path_trailer" />
